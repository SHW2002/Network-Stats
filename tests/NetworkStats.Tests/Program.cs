...
[
    ...
    ("周期网速检测默认关闭且不读取响应体", UrlProbeTests.DisabledByDefaultUsesHeadersOnlyAsync),
    ("响应等待独立计时、响应体下载速度和小样本标记", TransferTimingTests.SeparateWaitingAndDownloadingAsync),
    ("缓冲读取虚高速度与最低有效采样边界", DownloadSampleTests.BufferedRatesAndBoundariesAsync),
    ("直连及代理小网页不产生虚假速度，保留访问耗时", DownloadSampleTests.SmallResponsesThroughProxiesAsync),
    ("历史缓冲读取速度重新判定且原始数据保持完整", DownloadSampleTests.BufferedHistoryAsync),
    ...
    ("主题、窗口及启动设置持久化与默认网站", PreferenceTests.DefaultsAndPersistenceAsync),
    ("每次探测记录、历史保留、断行恢复和无探测空白", StorageTests.HistoryBucketsAndRecoveryAsync),
    ("时间图等待结果后推进、跨分钟归档及真实缺测", TimelineTests.AdvanceOnlyWithPublishedResultsAsync),
    ("整轮结果同时发布、历史保留与重启恢复", TimelineTests.PublishWholeRoundAsync),

...
用时间线查看网站是否可访问和响应耗时，并可按需记录响应体下载速度，持续观察直连与代理线路的网络表现。
...
2. 打开 **设置**，添加要监测的网站和代理地址，保存后自动开始探测；周期网速检测默认关闭，可手动开启，并可分别为直连和每个代理设置开关，开关旁会显示预计每小时最大流量；直连始终保留，本机代理通常填写 `127.0.0.1` 和代理软件提供的端口。
3. 查看时间线：绿色表示正常、黄色表示较慢、红色表示不可访问，空格表示没有数据；响应耗时和已开启的网速数据使用相同状态色，关闭网速检测时隐藏测速行；可切换最近 1、3、6、24 小时，点击色块查看详情。
4. 使用 **立即探测** 刷新整组结果，或进入 **单项测速** 复测一个网站；主界面的流量指标会分别显示 Ping 探测与周期测速的每小时预估消耗；在 **设置 → 软件更新** 中检查并安装新版本，也可单独配置更新代理。

Windows 最小化后会在托盘继续监测，双击托盘图标恢复，右键可退出；设置中可调整深浅色主题、开机自启动及关闭窗口时的行为。

测速反映目标 URL 的响应体传输情况，不能直接当作宽带峰值；周期测速关闭时只获取响应头，单项测速仍可随时使用；样本不足时会明确提示，配置和历史记录保存在本机。

## 开发框架

- **.NET 10 + .NET MAUI**：原生界面，支持 Windows、Android、iOS 和 macOS（Mac Catalyst）工程。
- **GraphicsView**：按每次已完成探测绘制网络状态时间图；时间轴根据所选范围自动选择整 10 分钟倍数的间隔，最左侧显示普通时间刻度，所有刻度保持等距，最右侧固定显示“现在”；Windows 界面基于 WinUI 3。

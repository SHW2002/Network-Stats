...
internal static class ProbePresentation
{
    public static string Speed(ProbeResult? sample) => sample switch
    {
        ...
        { Transfer: null } => "旧记录无速度",
        { Transfer.BytesReceived: 0 } => "无响应体",
        _ => Rate(sample.Transfer!.MegabytesPerSecond)
    };
    ...
    public static string Describe(CellSelection selection)
    {
        ...
        var text = $"{prefix}\n{Ui.StatusText(sample.Status)} · 访问耗时 {sample.LatencyMs:N0} ms" +
            (sample.HttpStatus is { } code ? $" · HTTP {code}" : "") +
            $" · 采样于 {sample.CheckedAt.ToLocalTime():HH:mm:ss}";
        if (sample.Transfer is { } transfer)
        {
            text += $"\n访问速度：{Speed(sample)} · 已读取 {transfer.BytesReceived / 1000.0:N1} KB";
            if (sample.Status != ProbeStatus.Unreachable && transfer.BytesReceived > 0)
            ...
            text += "\n速度按响应体字节数 / 整次请求耗时计算，仅代表这个 URL 的单次访问表现。";
        }

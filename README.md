...
ChatGPT 使用 `https://chatgpt.com/`，同样按配置 URL 的 HTTP 响应测速。该地址可能返回 Cloudflare 浏览器验证，程序识别响应中的 `cf-mitigated: challenge` 标记后会说明“站点要求浏览器验证”，不将验证页作为目标页面计算下载速度，即使验证响应为 HTTP 200。
自动探测不能执行浏览器验证或复用浏览器的登录状态，修改 User-Agent 也不保证通过。可在浏览器中使用同一线路检查能否访问；浏览器能打开而程序收到验证页时，该 URL 的自动下载测速仍不可用。程序保留原 URL，不替换成其他地址，也不会将 403 当作测速成功。

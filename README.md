...
3. Read the timeline: green means healthy, yellow means slow, red means unavailable, and an empty cell means no data. Response time and enabled speed values use the same status colors; the speed row is hidden when periodic speed measurement is off. Select the last 1, 3, 6, or 24 hours and click a cell for details.

...
- 默认检测 `https://baidu.com/`、`https://google.com/`、`https://github.com/`、`https://pixiv.net/`、`https://chatgpt.com/`，可在设置中增删网站。
...
- Windows 最小化后隐藏到托盘并继续探测，双击托盘恢复，右键选择显示或退出；可在设置中开启“关闭时改为最小化”，默认关闭。
...
主界面顶部显示 **v1.2.0**；启动后自动开始一轮 URL 测速，完成后网站名称下方显示下载速度和总耗时，也可以点击“立即测速”。
...
在 **设置 → 外观与窗口** 中选择深色模式（跟随系统 / 浅色 / 深色），或切换 Windows 的 **“关闭时改为最小化”**，然后点击“保存设置”。
...
ChatGPT 使用 `https://chatgpt.com/`，同样探测配置的原始 URL。Windows 检测到 `cf-mitigated: challenge` 后，会尝试使用已安装的 **Microsoft Edge** 在独立 Windows 桌面中访问原 URL；不显示窗口、不切换桌面、不读取用户的浏览器配置或登录 Cookie。
浏览器遵守同一条直连／代理线路，禁用缓存和 Service Worker，使用页面性能计量中的正文实际传输字节（压缩时为压缩字节）与首字节到正文结束的时间计算速度，界面和历史详情标记“后台浏览器”。不会把验证页、错误页、脚本、图片或浏览器所有流量当作目标正文速度。
HTTP 与浏览器重试共用设置中的整体请求时限，成功后该 URL／线路在本次运行中直接使用浏览器探测；最多同时运行两个浏览器，排队和准备时间计入总耗时。若频繁超时，可在设置中将请求超时调整到 20–30 秒。
浏览器以独立临时配置运行，结束或取消时清理整个进程组；不会执行验证码交互。仍需人工验证、没有 Edge、超时或下载未完成时如实显示失败，不能保证所有站点或网络出口都通过防护。
浏览器为限制内存和流量，在正文解压后的大小超过采样上限时也会停止；只有完整正文才能提供有效的浏览器速度，验证脚本等额外请求不计入正文采样字节。正常页面正文开始返回后阻止额外子资源下载。
Android、iOS、macOS 暂时继续使用普通 HTTP 探测；macOS 自启动与浏览器探测是独立功能。不会把 403 当作成功，也不会替换配置 URL。
...
已有配置不会被新默认列表覆盖；升级前已保存网站列表的设备可在设置中手动添加 ChatGPT。
...
# Windows / Edge：真实浏览器、代理与自启动注册测试，所有窗口位于隔离桌面。
dotnet run --project tests/NetworkStats.BrowserTests/NetworkStats.BrowserTests.csproj
# 或使用项目内独立 SDK：
.\scripts\test-windows-browser.ps1
```

...
普通关闭、托盘退出和更新重启均等待探测停止及历史写入完成；验包同时检查实际退出码和关闭顺序，界面报告通过但退出异常仍会拒绝发布。
...
启动时立即采样，默认之后每分钟一轮；每个网站名称下方分别显示最近一次 **下载 KB/s / MB/s**（无法可靠计算时显示 **“样本不足”**）、**总耗时 ms** 和必要的小样本提示，点击分钟色块查看历史速度、时间明细、下载量和错误详情。
...
例如等待响应 800 ms、再用 500 ms 读取 500 KB 时，下载速度为 **1 MB/s**，访问总耗时为 **1300 ms**；连接与响应等待不会混入响应体下载速度。
只下载配置 URL 返回的内容，不加载网页中的图片、脚本或视频；小网页的内容可能已经部分进入网络缓冲。若 70 KB 内容在 0.1 ms 内读完，直接相除会得到虚假的 **700 MB/s**；现在这类结果显示“样本不足”，保留真实下载量和访问耗时，不展示缓冲区的瞬时读取速率。
...
- 下载量至少 **64 KB** 且响应体读取时间至少 **200 ms** 才计算速度；未达到时显示 **“样本不足”**，正常 HTTP 响应仍按访问总耗时判色。此门槛同样用于实时进度、单项测速和已保存的历史，不会用响应等待凑时长，也不会把速度截断成固定最大值。
- 达到上述最低门槛，但下载量不足 **1 MB** 或读取时间不足 **2 秒**时标注 **“样本较小 · 仅供参考”**；判断某个站点的持续下载能力需要它提供足够大的响应内容。高速线路可能很快读完 1 MB 上限，此时也只能说明本次样本不足。
...
URL 自动测速检查还覆盖原始路径与查询参数、流量和时限上限、合法空响应、响应等待与下载计时分离、小样本提示、缓冲读取虚高速度、直连与 HTTP / SOCKS5 小响应，以及 v1.0 / v1.1 / 新版历史记录兼容。

...
- 手动下载测速：选择直连或已配置代理，用实际响应体下载计算 Mbps / MB/s，显示流量和耗时，支持自定义下载地址及随时取消。
...
验证包含一次空解包缓存启动、两次复用缓存启动，以及真实控件模板加载、时间图绘制、设置页往返、下载测速页的回环下载和托盘初始化。
测试只访问回环地址，使用独立数据目录；报告（包含包的 SHA256 和启动耗时）保存到 `artifacts/windows-package-check.txt`。
测试桌面不会切换到前台，整个发布和验证流程都不会在当前桌面弹出或激活窗口。
如果旧版正在运行、发布目录被占用，新版会保留在 `artifacts/windows-update/`（已有同名目录时加唯一后缀），脚本会输出实际路径；退出旧版后可以直接运行该目录中的 EXE。
...
点击主界面的 **“下载测速”**，选择线路并点击 **“开始测速”**。
默认使用 Cloudflare 的测试下载地址，也可以填写指定网站较大文件的 HTTP / HTTPS 直链；网站首页通常太小，难以测出持续下载速度。
测速使用所选的直连或 HTTP / HTTPS / SOCKS5 代理；直连显式禁用系统代理，代理失败不会退回直连。

每次最多读取 **20 MB 响应体**，从发起请求起最多 **10 秒**，达到任一上限即结束；网络缓冲和协议开销可能产生少量额外流量。
支持随时取消，离开测速页或移动端进入后台会取消当前测试，下载内容只用于计数、不保存到磁盘。

- 速率 = 实际收到的响应体字节数 ÷ 从响应头返回到读取结束的时间；包含响应体传输中的等待，连接和响应头耗时另计入“总耗时”。
- 使用十进制单位：`8 Mbps = 1 MB/s`，`1 MB = 1,000,000 字节`；请求不使用压缩，服务端仍压缩时按实际收到的压缩字节计量。
- 显示当前测试的平均速率；样本不足 1 MB 或传输不足 2 秒会提示样本较短。
- 结果反映当前设备经选定线路到具体服务器 / CDN 的单连接下载速度，不能直接代表宽带套餐上限、上传速度或其他网站的速度。
- 分钟时间图继续记录访问响应耗时，手动测速结果显示在测速页；测速占用网络时，可能影响同期的响应耗时。
...
单文件包额外验证真实 MAUI 窗口、WinUI 控件模板、Win2D 时间图绘制、设置页往返、测速页回环下载及托盘集成初始化；隔离桌面没有 Explorer 托盘，托盘菜单交互仍需手动验收。

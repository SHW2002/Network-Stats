...
主界面顶部显示 **v1.1.1**；启动后自动开始一轮 URL 测速，完成后网站名称下方显示下载速度和总耗时，也可以点击“立即测速”。
...
单文件包额外验证真实 MAUI 窗口、WinUI 控件模板、Win2D 时间图绘制、设置页往返、配置 URL 的自动采样及主界面速度显示、单项复测、分别显示下载速度与耗时、小样本提示和托盘集成初始化；隔离桌面没有 Explorer 托盘，托盘菜单交互仍需手动验收。
